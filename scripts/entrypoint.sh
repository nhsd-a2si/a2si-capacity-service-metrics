...
test_postgresql() {
  ...
  echo "JP3 - Connecting to postgres 2"
  export PGPASSWORD=FWr2qN3WTdZJ
#  psql -h ${DB_HOST:-postgres} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} ${DB_NAME:-jasperserver} $@
psql -h a2si-cd-stage-capacity-service-db.ckg0gmxsawjt.eu-west-2.rds.amazonaws.com -p 5432 -U postmanpat a2sicdstageCapacityService $@

}

retry_postgresql() {
  # Retry 5 times to check PostgreSQL is accessible.
  echo "JP3 - Connecting to postgres"
  for retry in {1..5}; do
  ...
  test_postgresql || \
    ...
    echo "JP3 - Error: PostgreSQL on 10.1.0.218 not accessible!"
}
